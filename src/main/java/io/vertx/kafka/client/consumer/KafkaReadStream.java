...
public interface KafkaReadStream<K, V> extends ReadStream<ConsumerRecord<K, V>> {
   ...
   * @return  current KafkaReadStream instance
   */
  KafkaReadStream<K, V> subscribe(Set<String> topics, Handler<AsyncResult<Void>> completionHandler, TracingKafkaConsumer tracer);
   ...
   * @param tracer  opentracing tracer
   */
  void poll(Duration timeout, Handler<AsyncResult<ConsumerRecords<K, V>>> handler, TracingKafkaConsumer tracer);
}
